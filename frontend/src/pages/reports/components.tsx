...
export function BarTable({ title, rows }: {
  ...
  const max = Math.max(...safeRows.map(r => r.count), 1);
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-5">
...
export function MiniLineChart({ title, data }: {
  ...
  const w = 4;
  const h = 60;
  const pts = safeData.map((d, i) => `${i * w},${h - (d.count / max) * h}`).join(' ');
    ...
    <div className="bg-white border border-gray-200 rounded-xl p-5">
      ...
      {safeData.length === 0 ? (
        ...
        <div className="overflow-x-auto">
          <svg width={safeData.length * w} height={h + 4} className="min-w-full">
            <polyline
              points={pts}
              fill="none"
              stroke="#3b82f6"
          ...
          <div className="flex justify-between text-xs text-gray-400 mt-1">
            <span>{safeData[0]?.date}</span>
            <span>{safeData[safeData.length - 1]?.date}</span>
          </div>
        </div>
      )}
    </div>
