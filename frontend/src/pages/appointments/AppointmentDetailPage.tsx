...
export default function AppointmentDetailPage() {
  ...
  useEffect(() => {
    ...
    getAppointment(id)
      ...
      .catch((err: unknown) => {
        ...
        setLoadError(msg || 'Failed to load appointment.');
      })
  ...
  async function handleTransition(toStatus: string) {
    ...
    try {
      ...
      const msg = (err as { response?: { data?: { message?: string } } })?.response?.data?.message;
      setTransitionError(msg || 'Transition failed.');
    } finally {
      setTransitioning('');
    }
  }
  ...
  async function handleCancel() {
    ...
    try {
      ...
      const msg = (err as { response?: { data?: { message?: string } } })?.response?.data?.message;
      setTransitionError(msg || 'Cancel failed.');
    } finally {
      setTransitioning('');
    }
  }
  ...
  return (
    <div className="p-6 max-w-3xl">
      ...
      {transitionError && (
        <div className="mb-4 bg-red-50 border border-red-200 text-red-600 rounded-lg px-4 py-3 text-sm">{transitionError}</div>
      )}
